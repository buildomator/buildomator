...
- Read frontmatter `status` field via:
  ```bash
  bm-sdk query frontmatter.get .planning/threads/{file} status
  ```
...
3. Commit:
   ```bash
   bm-sdk query commit "docs: resolve thread — {SLUG}" --files ".planning/threads/{SLUG}.md"
   ```
...
5. Commit:
   ```bash
   bm-sdk query commit "docs: create thread — ${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
