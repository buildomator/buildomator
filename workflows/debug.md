...
Systematic debugging (scientific method, subagent isolation): orchestrates symptom gathering, session creation, and delegation to `bm-debug-session-manager`.

<available_agent_types>
Valid GSD subagent types (use exact names — do not fall back to 'general-purpose'):
- gsd:bm-debug-session-manager — manages debug checkpoint/continuation loop in isolated context
- gsd:bm-debugger — investigates bugs using scientific method
</available_agent_types>
...
```bash
TDD_MODE=$(bm-sdk query config-get workflow.tdd_mode 2>/dev/null | jq -r 'if type == "boolean" then tostring else . end' 2>/dev/null || echo "false")
```
...
""",
  subagent_type="gsd:bm-debug-session-manager",
