...
tools: Read, Write, Edit, Bash, Grep, Glob, mcp__context7__*, Skill
color: yellow
# hooks:
#   PostToolUse:
#     - matcher: "Write|Edit"
...
@${CLAUDE_PLUGIN_ROOT}/references/ios-scaffold.md
...
```bash
AUTO_CHAIN=$(bm-sdk query config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(bm-sdk query config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**See @${CLAUDE_PLUGIN_ROOT}/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `${CLAUDE_PLUGIN_ROOT}/references/execute-mvp-tdd.md` (Read it — lazy, only on MVP+TDD runs). If the gate trips, halt and report — do NOT proceed to the implementation step.
...
```bash
bm-sdk query commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
<state_updates>
After SUMMARY.md, update STATE.md using `bm-sdk query` state handlers (positional args; see `sdk/src/query/QUERY-HANDLERS.md`):
...
for decision in "${DECISIONS[@]}"; do
  bm-sdk query state.add-decision "${decision}"
done
...
bm-sdk query roadmap.update-plan-progress "${PHASE_NUMBER}"
...
```bash
bm-sdk query state.add-blocker "Blocker description"
```
</state_updates>

<final_commit>
```bash
bm-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" --files \
