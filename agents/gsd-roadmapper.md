...
Reference full template: `${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/plugins/cache/gsd-plugin/current}/templates/roadmap.md`

## STATE.md Structure

Use template from `${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/plugins/cache/gsd-plugin/current}/templates/state.md`.
...
User can review actual files in the editor or via SDK queries (e.g. `bm-sdk query roadmap.analyze` and `bm-sdk query state.load`) instead of ad-hoc shell `cat`.
