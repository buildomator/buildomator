...
export class PhaseRunnerError extends Error {
    ...
    step;
    cause;
    constructor(message, phaseNumber, step, cause) {
        super(message);
...
export class PhaseRunner {
    ...
    async run(phaseNumber, options) {
        ...
        try {
            ...
            throw new PhaseRunnerError(`Failed to initialize phase ${phaseNumber}: ${err instanceof Error ? err.message : String(err)}`, phaseNumber, PhaseStepType.Discuss, err instanceof Error ? err : undefined);
        }
        // Validate phase exists
        if (!phaseOp.phase_found) {
            throw new PhaseRunnerError(`Phase ${phaseNumber} not found on disk`, phaseNumber, PhaseStepType.Discuss);
        }
     ...
     */
    async runPlanCheckStep(phaseNumber, sessionOpts) {
        ...
        try {
            ...
            planResult = await runPhaseStepSession(prompt, PhaseStepType.PlanCheck, this.config, sessionOpts, this.eventStream, { phase: PhaseType.Verify, planName: undefined });
        }
        catch (err) {
     ...
     */
    async runSelfDiscussStep(phaseNumber, sessionOpts) {
        const stepStart = Date.now();
        this.eventStream.emitEvent({
            type: GSDEventType.PhaseStepStart,
            timestamp: new Date().toISOString(),
        ...
        try {
            ...
            planResult = await runPhaseStepSession(prompt, PhaseStepType.Discuss, this.config, sessionOpts, this.eventStream, { phase: PhaseType.Discuss, planName: undefined });
        }
        catch (err) {
            const durationMs = Date.now() - stepStart;
            const errorMsg = err instanceof Error ? err.message : String(err);
     ...
     */
    async runStep(step, phaseNumber, sessionOpts) {
        const stepStart = Date.now();
        this.eventStream.emitEvent({
            type: GSDEventType.PhaseStepStart,
            timestamp: new Date().toISOString(),
        ...
        try {
            ...
            planResult = await runPhaseStepSession(prompt, step, this.config, sessionOpts, this.eventStream, { phase: phaseType, planName: undefined });
        }
        catch (err) {
            const durationMs = Date.now() - stepStart;
            const errorMsg = err instanceof Error ? err.message : String(err);
    ...
    async runExecuteStep(phaseNumber, sessionOpts) {
        ...
        if (this.config.parallelization === false) {
            ...
            for (const waveNum of sortedWaves) {
                ...
                const settled = await Promise.allSettled(wavePlans.map(plan => this.executeSinglePlan(phaseNumber, plan.id, sessionOpts)));
     ...
     */
    async executeSinglePlan(phaseNumber, planId, sessionOpts) {
        try {
            ...
            return await runPhaseStepSession(prompt, PhaseStepType.Execute, this.config, sessionOpts, this.eventStream, { phase: phaseType, planName: planId });
        }
        catch (err) {
            return {
                success: false,
     ...
     */
    async runVerifyStep(phaseNumber, sessionOpts, callbacks, options) {
        const stepStart = Date.now();
        this.eventStream.emitEvent({
            type: GSDEventType.PhaseStepStart,
            timestamp: new Date().toISOString(),
        ...
        let lastResult;
        let outcome = 'passed';
        const allPlanResults = [];
        while (true) {
            try {
                const phaseType = PhaseType.Verify;
                const contextFiles = await this.contextEngine.resolveContextFiles(phaseType);
                const prompt = await this.promptFactory.buildPrompt(phaseType, null, contextFiles);
                lastResult = await runPhaseStepSession(prompt, PhaseStepType.Verify, this.config, sessionOpts, this.eventStream, { phase: phaseType });
     ...
     */
    async runAdvanceStep(phaseNumber, _sessionOpts, callbacks) {
        const stepStart = Date.now();
        this.eventStream.emitEvent({
            type: GSDEventType.PhaseStepStart,
            timestamp: new Date().toISOString(),
     ...
     */
    async parseVerificationOutcome(result, phaseNumber) {
        // If the session itself crashed, that's a clear failure
        if (!result.success) {
            if (result.error?.subtype === 'human_review_needed')
                return 'human_needed';
            return 'gaps_found';
        }
     ...
     */
    async invokeBlockerCallback(callbacks, phaseNumber, step, error) {
     ...
     */
    async invokeVerificationCallback(callbacks, phaseNumber, stepResult) {
