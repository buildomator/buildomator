...
export class InitRunner {
    ...
    async run(input) {
        ...
        try {
            ...
            const projectInfo = setupResult.value;
    ...
    // ─── Parallel research ─────────────────────────────────────────────────────
    async runParallelResearch(input, projectInfo) {
        this.emitEvent({
        ...
        const promises = RESEARCH_TYPES.map(async (researchType) => {
            ...
            const result = await this.runStep(step, async () => {
                ...
                if (!sessionResult.success) {
                    throw new Error(`Research (${researchType}) failed: ${sessionResult.error?.messages.join(', ') ?? 'unknown error'}`);
                }
     ...
     */
    async buildProjectPrompt(input) {
    ...
    async buildRequirementsPrompt() {
        ...
        try {
            featuresContent = await readFile(join(this.projectDir, '.planning', 'research', 'FEATURES.md'), 'utf-8');
        }
        catch {
     ...
     */
    async runSession(prompt, modelOverride) {
        const config = await loadConfig(this.projectDir);
        return runPhaseStepSession(prompt, PhaseStepType.Research, // Research phase gives broadest tool access
        config, {
            ...
            cwd: this.projectDir,
        }, this.eventStream, { phase: undefined, planName: undefined });
    }
    ...
    // ─── Result helpers ────────────────────────────────────────────────────────
    buildResult(success, steps, artifacts, startTime) {
