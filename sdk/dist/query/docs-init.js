 ...
 */
export function scanExistingDocs(cwd) {
    const MAX_DEPTH = 4;
    const results = [];
...
export function detectDocTooling(cwd) {
    const exists = (rel) => pathExistsInternal(cwd, rel);
    return {
...
export function detectMonorepoWorkspaces(cwd) {
    try {
    ...
    try {
        const lerna = JSON.parse(readFileSync(join(cwd, 'lerna.json'), 'utf-8'));
