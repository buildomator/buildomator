 ...
 */
export function stripShippedMilestones(content) {
 ...
 */
async function parseMilestoneFromState(projectDir, workstream) {
    try {
 ...
 */
export async function getMilestoneInfo(projectDir, workstream) {
    try {
...
export async function extractCurrentMilestone(content, projectDir, workstream) {
    ...
    if (!sectionMatch || sectionMatch.index === undefined) {
        ...
        const detailsPattern = new RegExp(`<details\\b[^>]*>\\s*<summary\\b[^>]*>` +
    ...
    if (sectionEnd < content.length) {
        ...
        if (phaseDetailsMatch && phaseDetailsMatch.index !== undefined) {
            ...
            const nextRealMilestoneRegex = new RegExp(`^#{1,${pdLevel}}\\s+(?!Phase\\s+\\S)(?!Phase\\s+Details\\b)(?:.*v(\\d+(?:\\.\\d+)+)[^\\n]*|.*(?:✅|📋|🚧|🟡))`, 'gmi');
            let pdEnd = content.length;
            let mm;
            while ((mm = nextRealMilestoneRegex.exec(afterPdHeading)) !== null) {
                const mv = mm[1];
                if (mv && currentVersionStr && mv === currentVersionStr)
                    continue;
 ...
 */
export function extractPhasesFromSection(section) {
 ...
 * "next" one.
 */
export async function extractNextMilestoneSection(content, projectDir) {
    ...
    if (!currentVersion) {
        ...
        if (inProgressMatch)
            currentVersion = 'v' + inProgressMatch[1];
    }
    if (!currentVersion)
        return null;
    // Find the current milestone ## heading.
    const escaped = escapeRegex(currentVersion);
    const currentHeadingPattern = new RegExp(`^##\\s+[^\\n]*${escaped}[^\\n]*$`, 'mi');
...
async function countPhasePlansAndSummaries(phaseDir) {
 ...
 */
export const roadmapGetPhase = async (args, projectDir, workstream) => {
    const phaseNum = args[0];
    if (!phaseNum) {
        throw new GSDError('Usage: roadmap get-phase <phase-number>', ErrorClassification.Validation);
    }
 ...
 */
export const roadmapAnalyze = async (_args, projectDir, workstream) => {
    const roadmapPath = planningPaths(projectDir, workstream).roadmap;
    let rawContent;
    try {
        rawContent = await readFile(roadmapPath, 'utf-8');
 ...
 */
export const roadmapAnnotateDependencies = async (args, projectDir) => {
    const phase = args[0];
    if (!phase) {
 ...
 */
export const requirementsMarkComplete = async (args, projectDir, workstream) => {
    if (args.length === 0) {
        throw new GSDError('requirement IDs required. Usage: requirements mark-complete REQ-01,REQ-02 or REQ-01 REQ-02', ErrorClassification.Validation);
    }
    const reqIds = args
        .join(' ')
        .replace(/[\[\]]/g, '')
