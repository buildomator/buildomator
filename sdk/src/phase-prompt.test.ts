...
describe('PromptFactory', () => {
  ...
  describe('buildPrompt', () => {
    it('assembles research prompt with role + purpose + process + context', async () => {
      ...
      await writeFile(
        join(agentsDir, 'bm-phase-researcher.md'),
        makeAgentDef('bm-phase-researcher', 'Read, Grep, Bash', 'You are a researcher.'),
      );
    ...
    it('assembles plan prompt with all context files', async () => {
      ...
      await writeFile(
        join(agentsDir, 'bm-planner.md'),
        makeAgentDef('bm-planner', 'Read, Write, Bash', 'You are a planner.'),
      );
    ...
    it('delegates execute phase with plan to buildExecutorPrompt', async () => {
      await writeFile(
        join(agentsDir, 'bm-executor.md'),
        makeAgentDef('bm-executor', 'Read, Write, Edit, Bash', 'You are an executor.'),
      );
    ...
    it('assembles verify prompt with phase instructions', async () => {
      ...
      await writeFile(
        join(agentsDir, 'bm-verifier.md'),
        makeAgentDef('bm-verifier', 'Read, Bash, Grep', 'You are a verifier.'),
      );
    ...
    it('handles missing workflow file gracefully', async () => {
      ...
      await writeFile(
        join(agentsDir, 'bm-phase-researcher.md'),
        makeAgentDef('bm-phase-researcher', 'Read, Bash', 'You are a researcher.'),
      );
  ...
  describe('loadAgentDef', () => {
    ...
    it('falls back to project agents dir', async () => {
      ...
      await writeFile(
        join(projectAgentsDir, 'bm-executor.md'),
    ...
    it('prefers user agents dir over project agents dir', async () => {
      const projectAgentsDir = join(tempDir, 'project-agents');
      await mkdir(projectAgentsDir, { recursive: true });
      await writeFile(join(agentsDir, 'bm-executor.md'), 'user agent');
      await writeFile(join(projectAgentsDir, 'bm-executor.md'), 'project agent');
  ...
  describe('headless prompt loading', () => {
    ...
    it('loadAgentDef prefers installed agents over sdkPromptsDir', async () => {
      ...
      await writeFile(join(agentsDir, 'bm-executor.md'), 'user agent');
      await writeFile(join(sdkDir, 'agents', 'bm-executor.md'), 'SDK bundled agent');
