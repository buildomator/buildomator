...
2. **Cron (hourly, currently the ACTIVE deployer):** `0 * * * * /var/www/buildomator.com/rebuild-buildomator.sh` (root-owned loose file). It does `git pull`, `npm run build` (NO `npm ci`), then `sudo rm -rf current/*` + `cp dist/* current/` + `chown -R www-data current`. This makes `current` a real directory.
