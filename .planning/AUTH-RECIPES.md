...
2. **Cron (hourly, currently the ACTIVE deployer):** `0 * * * * /var/www/buildomator.com/rebuild-buildomator.sh >> /var/www/buildomator.com/deploy.log 2>&1 || echo "buildomator hourly rebuild failed; see .../deploy.log"` (root-owned loose file). It does `git pull`, `npm run build` (NO `npm ci`), then `sudo rm -rf current/*` + `cp dist/* current/` + `chown -R www-data current`. This makes `current` a real directory. As of 2026-07-25 the cron line redirects all output to `deploy.log` and emails only on a non-zero exit, so successful/no-op runs no longer mail "Already up to date" while a failure still emails. Also on 2026-07-25 the script's build line was fixed from `npm run build 2>&1 | grep ... || true` (which swallowed build failures) to plain `npm run build`, so under `set -e` a failed build now aborts BEFORE the destructive `rm -rf current/*` (no outage) and exits non-zero, which triggers the cron failure email with the build error captured in `deploy.log`. A backup of the pre-fix script is kept alongside it (`rebuild-buildomator.sh.bak.<ts>`).
...
- **The active hourly script is a root-owned loose copy (not the repo-tracked `source/` one).** Two of its regressions have since been patched in place (2026-07-25): a failed build now aborts before `rm -rf current/*` (no outage) and emails, via the plain `npm run build` + `set -e` fix above. STILL outstanding: it runs `npm run build` with NO `npm ci`, so a newly added dependency is not installed (the build would then fail, which now at least emails rather than silently shipping stale output). Restoring `npm ci` and tracking the script in the repo remain part of the consolidation below.
