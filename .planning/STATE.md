...
| 260608-vk9 | Retarget the scheduled upstream-release notification (`bin/check-gsd-release.sh`) from the locked `gsd-build/get-shit-done` to canonical `open-gsd/gsd-core` (npm @opengsd/gsd-core; redux mirrors tags). The watch was silently dead since the old repo ships no releases. Synced the live cron copy at ~/claude-code-gsd + reseeded ~/.gsd-last-known-version to 1.4.0 so the switch fires no spurious cross-numbering email; arms on next genuine release. check-upstream-schema.cjs still points old (deferred to sync work) | 2026-06-08 | 649dab4 | Done | [260608-vk9-retarget-upstream-release-notification-t](./quick/260608-vk9-retarget-upstream-release-notification-t/) |
