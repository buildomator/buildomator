...
> AI design contract generated by `/gsd:ai-integration-phase`. Consumed by `bm-planner` and `bm-eval-auditor`.
...
> Researched by `bm-domain-researcher`. Grounds the evaluation strategy in domain expert knowledge.
...
> Fetched from official docs by `bm-ai-researcher`. Distilled for this specific use case.
...
> Written by `bm-ai-researcher`. Cross-cutting patterns every developer building AI systems needs — independent of framework choice.
