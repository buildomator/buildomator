...
|---------|--------------|----------------|
| **Buildomator identity + additive `/bm:` command surface** (v4.1.0) | The project is now Buildomator, and every command answers to `/bm:` as well as `/bm:`. Both prefixes run the same code side by side through the whole 4.x line, and `/bm:` retires at v5.0. Ships as a second generated plugin, so current installs keep working with nothing to re-enable. | `/bm:*` (all commands), `dist/bm/`, [buildomator.com](https://buildomator.com) |
