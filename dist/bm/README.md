...
1. Start a new project: `/bm:new-project`
2. Plan your first phase: `/bm:plan-phase`
...
**v3.7.1**: fixes `/bm:version` reporting `Error: Exit code 1` when you are up to date (the update-hint chain exited non-zero; it printed correctly but Claude Code flagged it), and trims its bash. Extends the resilience sweep to `/bm:validate-phase` and `/bm:secure-phase` so the fix-it action is recommended (not a neutral menu).
...
**v3.6.2**: corrects the "how to update" instructions (the `/plugins` Marketplace refresh flow, in the README and in `/bm:version`), and makes `/bm:version` leaner and node-independent so it works even when node is broken.

**v3.6.1**: fixes `/bm:profile-user` and dev-preferences failing with "Template not found" in installs (the bundled SDK resolved templates at a stale nested path), and aligns the `commit_docs` new-project default between the CJS and SDK resolvers.
...
- **Auto-resume across `/compact`**: PreCompact hook writes `.planning/HANDOFF.json`; on the next session, SessionStart auto-invokes `/bm:resume-work` so Claude continues at the same phase/plan/task with zero manual intervention
...
|---------|--------------|----------------|
| **Trusted roots for symlinked global skills** | A global skill whose directory is a symlink to a location outside the runtime skills dir is refused by default; listing that location in `agent_skills_security.trusted_global_roots` accepts it, while the filesystem root and your home directory are never accepted. | `.planning/config.json` |
...
| **Community-health advisory for public repos** (v4.3.0) | On a public GitHub repo missing a LICENSE and/or SECURITY.md, the health check proposes adding them: it reconciles a declared-but-fileless license mechanically, presents the copyleft-vs-permissive tradeoff when nothing is declared, and offers a private-disclosure SECURITY.md. Always proposes, never auto-writes; fails soft on private or non-GitHub repos. | `/bm:health`, `references/community-health-advisory.md` |
...
| **Per-language import-habit drift** (v4.9.0) | The convention gate flags Python wildcard and relative imports, Rust glob imports, and Go import ordering by majority vote, alongside the casing and module-system axes. Advisory, directory-scoped, test files excluded. | `gsd-tools verify conventions`, `bin/lib/conventions.cjs` |
...
| **Improved resilience when sessions are broken** (v3.7.0) | On a dropped/resumed session, interrupted UATs are never lost and coverage-tagging gaps auto-heal instead of interrogating you. | `/bm:next`, `workflows/plan-phase.md`, `workflows/verify-work.md`, `references/continuation-format.md` |
...
| **`/bm:version` command** (v3.4.9) | Prints the installed version and checks GitHub tags for the latest, showing update steps only when you are behind. Read-only, never fails the command. | `/bm:version` |
...
| **Claude Fable model tier** (v3.4.4; `claude-fable-5-1` as of v4.6.0) | A first-class `fable` model tier that the quality profile assigns to the heaviest agents. Anthropic-compatible runtimes resolve it to `claude-fable-5-1`; non-Claude runtimes alias it to their top model. | `sdk/shared/model-catalog.json` |
| **Auto-capture durable decisions in ad-hoc work** (v3.4.1) | `/bm:quick`, `/bm:debug`, `/bm:fast` save durable decisions to Claude Code memory at close-out, so context survives between milestones. Conservative by default. | `workflow.auto_memory_capture` |
| **Gaps auto-route by blocking severity** (v2.45.9; made automatic in v3.6.0) | Verifier `gaps_found` auto-routes by severity: goal-breaking gaps escalate to a follow-up phase, minor gaps park to backlog. No prompt. | `workflows/execute-phase.md`, `agents/bm-verifier.md` |
| **Amend docs into work commit** (v2.45.7) | `/bm:quick` folds PLAN/SUMMARY/STATE into the preceding work commit (one commit per task, not two), falling back to a separate commit when unsafe. | `workflows/quick.md` |
| **Executor self-test bias** (v2.45.6) | Nudges the executor toward automated self-checks instead of `human-verify` prompts that interrupt you. | `agents/bm-executor.md` |
...
| **User-docs prose ratchet** (v4.7.0) | Catches prose slop (em-dash and en-dash characters, filler marketing words, cliche openers) in the plugin's own README and CHANGELOG at commit time and in CI, against a frozen baseline that fails only on growth. Ported from the ASD-STE100 aerospace writing standard. | `bin/maintenance/check-user-docs-prose.cjs` |
| **Opus tier tracks the latest Claude flagship** (v2.45.1, `claude-opus-5-5` as of v4.9.1) | Resolves `opus` to `claude-opus-5-5` across the Anthropic-compatible runtimes (claude, opencode, copilot, hermes). Sonnet and Haiku follow the same pattern; non-Anthropic runtimes are unchanged. | `sdk/shared/model-catalog.json` |
| **Documentation-Driven Development mode** (v2.44.0) | Sibling to new-project: write a user-facing `docs/SPEC.md`, validate it, then derive phases from its sections. Best for CLIs, libraries, APIs. | `/bm:new-ddd` |
| **Auth-recipe memory** (v2.44.5) | Detects auth commands (gh/aws/ssh, credential env vars) via a hook with secret redaction; promote them to reusable per-project or cross-project recipes. | `/bm:remember-access` |
...
| **Scheduled resume** | Schedule a future Claude Code session to auto-run a GSD command at a set time (`HH:MM`, ISO, or `+2h`). Handy for quota windows. | `/bm:resume-at <time>` |
| **Auto-resume across `/compact`** | On compaction a `HANDOFF.json` is written and the next session auto-runs `/bm:resume-work` with zero manual input. | PreCompact + SessionStart hooks |
...
| Command IDs | `/gsd-<skill>` (dash; works because no namespace in upstream's flat install) | `/bm:<skill>` (colon; matches Claude Code's plugin namespace, autocompletes correctly) |

#### Session continuity (none of this exists upstream)

| Aspect | Upstream GSD | This plugin |
|--------|-------------|-------------|
| Behavior at `/compact` | Conversation collapses; phase position lost; user manually re-orients next session | **PreCompact hook** writes a 19-field `HANDOFF.json` capturing phase, plan, task, decisions, uncommitted files. **SessionStart hook** detects it on next session and auto-invokes `/bm:resume-work` with zero user input. |
| Periodic checkpoint mid-session | None; state only captured if user manually `/bm:pause-work`'s | **PostToolUse hook** writes a fresh checkpoint after most tool calls (`Bash`, `Edit`, `Write`, `MultiEdit`, `NotebookEdit`, `Read`, `Grep`, `Glob`, `WebFetch`, `WebSearch`), throttled to ≤1/min via mtime. Bridges Claude Code's silent *microcompact* path AND read-heavy research phases. After an unexpected session end (usage cap, kill, network drop), HANDOFF reflects state from at most ~60s before the cut. |
...
| Handoff lifecycle | N/A | `/bm:resume-work` deletes `HANDOFF.json` after a successful resume, preventing stale handoffs from triggering phantom resumes. |
...
| Namespace normalization | N/A; upstream commands ship in dash form | **`rewrite-command-namespace.cjs`** rewrites `/gsd-<skill>` to `/bm:<skill>` across all plugin content. Run after every upstream sync; drift is detected by the umbrella check. |
...
2. Run `/bm:help` -- should list all commands
3. Run `/bm:progress` -- shows project state (or prompts to create one)
4. Check MCP resources are available (the GSD MCP server should auto-start via plugin manifest)

## Upstream projects

Buildomator draws on four upstream projects, in three different ways: it **packages** one, **ports ideas from** two others, and **implements the format spec** of a third. None is a runtime dependency: the plugin bundles everything it needs.
...
**4. ASD-STE100 Simplified Technical English: a fourth, idea-only upstream.** Buildomator treats the 1986 aerospace controlled-language standard the way it treats VibeDrift: as a source of ideas to port, never a runtime component. [ASD-STE100 Simplified Technical English](https://en.wikipedia.org/wiki/Simplified_Technical_English) is the writing standard the aerospace industry has used for maintenance and flight documentation since the mid-1980s: a restricted vocabulary, one meaning per word, short sentences, and a mechanical structure that takes ambiguity out of a procedure a technician has to follow exactly. Buildomator ports the machine-checkable subset natively as the user-docs prose ratchet (`bin/maintenance/check-user-docs-prose.cjs`), which gates the plugin's own README and CHANGELOG against em-dash and en-dash characters, filler marketing words, and cliche openers, and fails a commit only when a count grows above a frozen baseline. The standard is a published specification, not code; Buildomator reads the rules and writes its own checker.
