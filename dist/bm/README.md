...
| **Claude Fable 5 model tier** (v3.4.4) | A first-class `fable` model tier that the quality profile assigns to the heaviest agents. Anthropic-compatible runtimes resolve it to `claude-fable-5`; non-Claude runtimes alias it to their top model. | `bin/lib/core.cjs`, `sdk/src/query/config-query.ts` |
