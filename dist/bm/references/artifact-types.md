...
- **Location**: `${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/plugins/cache/gsd-plugin/current}/USER-PROFILE.md`
...
### Spike README.md / MANIFEST.md (per-spike, via /bm:spike)
- **Shape**: YAML frontmatter (spike, name, validates, verdict, related, tags) + run instructions + results
- **Lifecycle**: Created by `/bm:spike` → Verified → Wrapped up by `/gsd-spike-wrap-up`
...
- **Lifecycle**: Created by `/bm:sketch` → Evaluated → Wrapped up by `/gsd-sketch-wrap-up`
