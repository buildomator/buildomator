...
| `manager.flags.discuss` | `""` | Flags passed to `/bm:discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
| `review.models.<cli>` | string\|null | `null` | Any model ID string | Per-CLI model override for /bm:review (e.g., `review.models.gemini`). Falls back to CLI default when null. |
...
| `workflow.ai_integration_phase` | boolean | `true` | `true`, `false` | Run /bm:ai-integration-phase before planning AI system phases |
...
| `workflow.security_enforcement` | boolean | `true` | `true`, `false` | Enable threat-model-anchored security verification via `/bm:secure-phase`. When `false`, security checks are skipped entirely |
...
Set via `ship.*` namespace in config.json. These fields affect `/bm:ship` PRD-style pull request body composition only.
...
Set via `intel.*` namespace (e.g., `"intel": { "enabled": true }`). Controls the queryable codebase intelligence system consumed by `/bm:map-codebase --query`.

| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `intel.enabled` | boolean | `false` | `true`, `false` | Enable queryable codebase intelligence system. When `true`, `/bm:map-codebase --query` builds and queries a JSON index in `.planning/intel/`. |
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/bm:discuss-phase` from manager (e.g., `"--auto --analyze"`) |
