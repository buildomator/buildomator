...
10. **NEVER** use non-GSD agent types (`general-purpose`, `Explore`, `Plan`, `Bash`, `feature-dev`, etc.) -- ALWAYS use `subagent_type: "gsd-{agent}"` (e.g., `bm-phase-researcher`, `bm-executor`, `bm-planner`). GSD agents have project-aware prompts, audit logging, and workflow context; generic agents bypass all of this.
