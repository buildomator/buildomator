...
**CJS-only (graphify):** `graphify` subcommands are not registered on `bm-sdk query`. Use `node ${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/plugins/cache/gsd-plugin/current}/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `bm-sdk query` where a handler exists.
...
Then run /bm:graphify build to create the initial graph.
```
...
- If no nodes found, display: `No graph matches for '<term>'. Try /bm:graphify build to create or rebuild the graph.`
...
```
PREFLIGHT=$(node "${CLAUDE_PLUGIN_ROOT:-$(ls -d "$HOME/.claude/plugins/cache/gsd-plugin/bm/"*/ 2>/dev/null|sort -V|tail -1)}/bin/gsd-tools.cjs" graphify build)
```
