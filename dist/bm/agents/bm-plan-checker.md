---
name: bm-plan-checker
description: Verifies plans will achieve phase goal before execution. Goal-backward analysis of plan quality. Spawned by /bm:plan-phase orchestrator.
...
Spawned by `/bm:plan-phase` orchestrator (after planner creates PLAN.md) or re-verification (after planner revises).
...
<upstream_input>
**CONTEXT.md** (if exists) — User decisions from `/bm:discuss-phase`
...
@${CLAUDE_PLUGIN_ROOT}/references/few-shot-examples/plan-checker.md
...
**Question:** Do plans honor user decisions from /bm:discuss-phase?
...
**If missing:** **BLOCKING FAIL** — "VALIDATION.md not found for phase {N}. Re-run `/bm:plan-phase {N} --research` to regenerate."
...
```bash
bm-sdk query phase.list-plans "$phase_number"
# Research / brief artifacts (deterministic listing)
bm-sdk query phase.list-artifacts "$phase_number" --type research
bm-sdk query roadmap.get-phase "$phase_number"
bm-sdk query phase.list-artifacts "$phase_number" --type summary
```
...
Use `bm-sdk query` to validate plan structure:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(bm-sdk query verify.plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using `bm-sdk query`:

```bash
MUST_HAVES=$(bm-sdk query frontmatter.get "$PLAN_PATH" must_haves)
```
...
```bash
PLAN_STRUCTURE=$(bm-sdk query verify.plan-structure "$PLAN_PATH")
```
...
```bash
bm-sdk query plan.task-structure "$PLAN_PATH"
```
...
```bash
bm-sdk query plan.task-structure "$PHASE_DIR/$PHASE-01-PLAN.md"
bm-sdk query frontmatter.get "$PHASE_DIR/$PHASE-01-PLAN.md" files_modified
```
...
Plans verified. Run `/bm:execute-phase {phase}` to proceed.
```
