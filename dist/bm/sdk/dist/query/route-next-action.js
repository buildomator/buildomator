...
async function hasUnresolvedVerificationFails(phaseDirAbs) {
    try {
        ...
        for (const line of lines) {
            if (/\|\s*FAIL\s*\|/i.test(line) && !/override/i.test(line))
                return true;
        }
        return false;
    }
...
async function verificationPassed(phaseDirAbs) {
    try {
        const files = await readdir(phaseDirAbs);
        const vf = files.find(f => f === 'VERIFICATION.md' || f.endsWith('-VERIFICATION.md'));
        if (!vf)
...
export const routeNextAction = async (_args, projectDir, workstream) => {
    ...
    if (currentPhase) {
        ...
        if (fd.found && fd.directory) {
            unresolvedVerification = await hasUnresolvedVerificationFails(join(projectDir, fd.directory));
        }
    }
    const gates = {
        continue_here: continueHere,
    ...
    const displayName = pd.phase_name || phaseName || '';
    const sorted = [...phases].sort((a, b) => comparePhaseNum(String(a.number), String(b.number)));
    if (!found) {
        const ctx = await buildContext(currentPhase);
        return {
            data: {
    ...
    const plans = pd.plans ?? [];
    const incomplete = pd.incomplete_plans ?? [];
    const hasContext = Boolean(pd.has_context);
    const hasResearch = Boolean(pd.has_research);
    const phaseDirAbs = pd.directory ? join(projectDir, pd.directory) : '';
    // Route 2
...
//# sourceMappingURL=route-next-action.js.map