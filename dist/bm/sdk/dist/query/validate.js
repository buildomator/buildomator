...
export const validateHealth = async (args, projectDir, workstream) => {
    ...
    if (existsSync(roadmapPath)) {
        try {
            ...
            const diskPhases = new Set();
            const activeDiskPhases = new Set();
            try {
                const entries = await readdir(phasesDir, { withFileTypes: true });
                for (const e of entries) {
                    if (e.isDirectory()) {
 ...
 */
function parseFlagInt(args, flag) {
...
export const validateContext = async (args, _projectDir) => {
    ...
    if (tokensUsed === null || tokensUsed < 0) {
        throw new GSDError('--tokens-used <non-negative integer> is required for `validate.context`', ErrorClassification.Validation);
    }
    if (contextWindow === null || contextWindow <= 0) {
        throw new GSDError('--context-window <positive integer> is required for `validate.context`', ErrorClassification.Validation);
    }
