...
export function assertSafePhaseDirName(dirName, label = 'phase directory') {
    if (/[/\\]|\.\./.test(dirName)) {
        throw new GSDError(`${label} contains invalid path segments`, ErrorClassification.Validation);
    }
}
export function assertSafeProjectCode(code) {
    if (code && /[/\\]|\.\./.test(code)) {
...
export function generatePhaseSlug(text) {
    return text
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '')
...
export function parseMultiwordArg(args, flag) {
 ...
 */
export function scanSequentialMaxPhaseFromMilestone(milestoneContent) {
 ...
 */
export function scanSequentialMaxPhaseFromDirs(dirNames) {
    let maxPhase = 0;
...
export function computeNextSequentialPhaseId(milestoneContent, dirNames) {
    return Math.max(scanSequentialMaxPhaseFromMilestone(milestoneContent), scanSequentialMaxPhaseFromDirs(dirNames)) + 1;
}
export function computePhaseDirectory(namingMode, descriptionSlug, prefix, nextSequentialPhaseId, customId) {
...
export function buildPhaseRoadmapEntry(phaseId, description, namingMode) {
...
export function collectDecimalSuffixesFromDirNames(basePhase, dirNames) {
...
export function collectDecimalSuffixesFromRoadmap(basePhase, roadmapContent) {
    const decimalSet = new Set();
...
export function computeNextDecimalPhase(basePhase, decimalSet) {
...
//# sourceMappingURL=phase-lifecycle-policy.js.map