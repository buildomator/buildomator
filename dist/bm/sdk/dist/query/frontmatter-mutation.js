...
function serializeArray(lines, key, arr, indent) {
    if (arr.length === 0) {
        ...
        arr.length <= 3 &&
        arr.join(', ').length < 60) {
        lines.push(`${indent}${key}: [${arr.join(', ')}]`);
    }
    else {
        lines.push(`${indent}${key}:`);
 ...
 */
export const frontmatterSet = async (args, projectDir) => {
    let filePath;
    let field;
    let value;
 ...
 */
export const frontmatterMerge = async (args, projectDir) => {
    ...
    let mergeData;
    try {
        mergeData = JSON.parse(jsonString);
    }
    catch {
 ...
 */
export const frontmatterValidate = async (args, projectDir) => {
    const filePath = args[0];
    ...
    if (!schema) {
        throw new GSDError(`Unknown schema: ${schemaName}. Available: ${Object.keys(FRONTMATTER_SCHEMAS).join(', ')}`, ErrorClassification.Validation);
    }
    let fullPath;
    try {
        fullPath = await resolvePathUnderProject(projectDir, filePath);
...
//# sourceMappingURL=frontmatter-mutation.js.map