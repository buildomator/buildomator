 ...
 */
export function buildExecutorPrompt(plan, agentDefOrOpts) {
    const opts = typeof agentDefOrOpts === 'string'
    ...
    // ── Plan metadata ──
    const meta = [];
    if (plan.frontmatter.phase)
        meta.push(`Phase: ${plan.frontmatter.phase}`);
    if (plan.frontmatter.plan)
        meta.push(`Plan: ${plan.frontmatter.plan}`);
    if (plan.frontmatter.type)
        meta.push(`Type: ${plan.frontmatter.type}`);
    if (meta.length > 0) {
