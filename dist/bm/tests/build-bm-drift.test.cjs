...
const SRC_MANIFEST = {
  ...
  mcpServers: { bm: { type: 'stdio', command: 'node', args: ['${CLAUDE_PLUGIN_ROOT}/mcp/server.cjs'] } },
};
...
check('no /bm: leaks in dist/bm text files outside COMMAND_REWRITE_EXCLUDE', () => {
  const leaks = [];
  function walk(dir, prefix) {
    for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      // COMMAND_REWRITE_EXCLUDE files legitimately retain /bm: (CHANGELOG.md
