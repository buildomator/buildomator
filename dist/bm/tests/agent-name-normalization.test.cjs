...
check('(g) parseCallsAgents and stageAgentsForProfile accept both prefixes', () => {
  const stems = installProfiles.parseCallsAgents('spawn bm:bm-planner and gsd-executor');
...
const legacySpawns = grepCount('subagent_type="bm:gsd-', spawnDirs);
if (legacySpawns > 0) {
  skip(`(h) ${legacySpawns} legacy subagent_type="bm:gsd-" refs remain — spawn-count check deferred to post-sweep`);
} else {
  check('(h) zero legacy spawns, 69 bm- spawns in workflows/skills/agents', () => {
    assert(grepCount('subagent_type="bm:gsd-', spawnDirs) === 0, 'legacy bm:gsd- spawns remain');
    const bmSpawns = grepCount('subagent_type="bm:bm-', spawnDirs);
    assert(bmSpawns === 69, `expected 69 bm:bm- spawns, got ${bmSpawns}`);
  });
}
