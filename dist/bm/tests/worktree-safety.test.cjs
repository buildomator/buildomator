...
// This test file (and its regenerated dist/bm copy) carries the retired literal
// as the detection string itself, so skip it by basename wherever it appears.
const THIS_BASENAME = path.basename(__filename);
...
check('no old narrow worktree-agent-only literal remains (dist/bm included)', () => {
  ...
  for (const f of files) {
    if (path.basename(f) === THIS_BASENAME) continue;
