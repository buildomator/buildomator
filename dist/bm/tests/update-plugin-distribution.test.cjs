...
//   4. transform sanity: the bm copy says /bm:update, the source says /bm:update.
//
...
check('transform sanity: source says /bm:update', () => {
  assert.ok(fs.readFileSync(SOURCE, 'utf8').includes('/bm:update'), 'source must retain /bm:update');
});

check('transform sanity: dist says /bm:update and never /bm:update', () => {
  const dist = fs.readFileSync(DIST, 'utf8');
  assert.ok(dist.includes('/bm:update'), 'dist must say /bm:update');
  assert.ok(!dist.includes('/bm:update'), 'dist must not contain /bm:update');
});
