...
- bm:bm-code-reviewer: Reviews source files for bugs and quality issues
</available_agent_types>
...
```
When **active**, run in ultracode mode: run EVERY review dimension regardless of the configured depth, then add an adversarial pass that tries to refute each finding before it is reported (drop findings that do not survive). When **inactive**, run normally. See `references/ultracode-mode.md`.
</ultracode_gate>
...
```
Error: Phase ${PHASE_ARG} not found. Run /bm:progress to see available phases.
```
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /bm:code-review ${PHASE_ARG} --files=file1,file2,..."
  fi
fi
```
...
No REVIEW.md created. You can retry with /bm:code-review ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  echo "No REVIEW.md to commit. Please retry with /bm:code-review ${PHASE_ARG}"
fi
```
</step>
...
Next steps:
  /bm:code-review ${PHASE_NUMBER} --fix  — Auto-fix issues
