...
```
UI phase is disabled in config. Enable via /bm:settings.
```
Exit workflow.
...
Recommended: run /bm:discuss-phase {N} first to capture design preferences.
...
⚡ Sketch findings detected: {SKETCH_FINDINGS_PATH}
   Validated design decisions from /bm:sketch will be loaded into the UI researcher.
...
- {SKETCH_FINDINGS_PATH} (Sketch Findings — validated design decisions, CSS patterns, visual direction from /bm:sketch, if exists)
</files_to_read>
...
Agent(
  ...
  subagent_type="bm:bm-ui-researcher",
...
Agent(
  ...
  subagent_type="bm:bm-ui-checker",
...
2. Edit manually — open UI-SPEC.md in editor, re-run /bm:ui-phase
...
`/clear` then: `/bm:plan-phase {N}`

{If CONTEXT.md does NOT exist:}
**Discuss Phase {N}** — gather implementation context before planning

`/clear` then: `/bm:discuss-phase {N}`

(or `/bm:plan-phase {N}` to skip discussion)
