...
```bash
INIT=$(bm-sdk query init.manager)
...
- `manager_flags.discuss` — appended to `/bm:discuss-phase` args (e.g. `"--auto --analyze"`)
- `manager_flags.plan` — appended to plan agent init command
- `manager_flags.execute` — appended to execute agent init command

These are empty strings by default. Set via: `bm-sdk query config-set manager.flags.discuss "--auto --analyze"`
...
All {phase_count} phases done. Ready for final steps:
  → /bm:verify-work — run acceptance testing
  → /bm:complete-milestone — archive and wrap up
```
...
```
 ...
 Resume anytime: /bm:manager
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/bm:manager` or `/bm:progress` invocation.
