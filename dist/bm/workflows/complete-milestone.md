...
[R] Resolve — stop and fix items, then re-run /bm:complete-milestone
...
2. **Run audit first** — `/bm:audit-milestone` to assess gap severity
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/bm:cleanup` later to archive retroactively.
...
- [ ] User knows next step (/bm:new-milestone)
