...
if [ ! -f "${REVIEW_PATH}" ]; then
  echo "Error: No REVIEW.md found for Phase ${PHASE_ARG}. Run /bm:code-review ${PHASE_ARG} first."
  exit 1
fi
```
...
You can retry with /bm:code-review ${PHASE_ARG} --fix.
```
...
if [ ! -f "${FIX_REPORT_PATH}" ]; then
  ...
  echo ""
  echo "Retry: /bm:code-review ${PHASE_ARG} --fix"
  echo ""
  exit 1
fi
```
...
if [ "$FIX_STATUS" = "all_fixed" ]; then
  ...
  echo ""
  echo "Next step:"
  echo "  /bm:verify-work  — Verify phase completion"
  echo ""
fi
```
...
if [ "$FIX_STATUS" = "partial" ] || [ "$FIX_STATUS" = "none_fixed" ]; then
  ...
  echo "  /bm:code-review ${PHASE_NUMBER}           — Re-review code"
  echo "  /bm:verify-work                           — Verify phase completion"
  echo ""
fi
```
