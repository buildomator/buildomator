...
```bash
USER_STORY_RESULT=$(bm-sdk query user-story.validate --story "$USER_STORY")
...
```bash
NEW_MODE=$(bm-sdk query roadmap.get-phase "${PHASE}" --pick mode)
NEW_GOAL=$(bm-sdk query roadmap.get-phase "${PHASE}" --pick goal)
```
