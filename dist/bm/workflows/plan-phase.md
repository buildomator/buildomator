...
**If `planning_exists` is false:** Error — run `/bm:new-project` first.
...
/bm:plan-phase --reviews cannot replan a closed phase. If the review surfaced
...
with: /bm:plan-phase ${phase_number} --force
...
if [ "$MVP_MODE" = "true" ] && [ "$padded_phase" = "01" ]; then
  PRIOR_SUMMARIES=$(bm-sdk query phases.list --pick summaries_total 2>/dev/null || echo "0")
  if [ "$PRIOR_SUMMARIES" = "0" ]; then WALKING_SKELETON=true; fi
fi
```

When `WALKING_SKELETON=true`:
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `${CLAUDE_PLUGIN_ROOT}/references/skeleton-template.md` (Read lazily, only on Walking-Skeleton runs).
...
```bash
CHUNKED_CFG=$(bm-sdk query config-get workflow.plan_chunked 2>/dev/null || echo "false")
CHUNKED_MODE=false
...
Then re-run /bm:plan-phase {N} --reviews
```
Exit workflow.
...
```bash
MVP_MODE=$(bm-sdk query phase.mvp-mode "${PHASE}" $MVP_FLAG_ARG --pick active)
```
...
5. Commit:
```bash
bm-sdk query commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
4. Empty-decisions fallback: if all parsed ADRs have zero `decisions[]`, emit `ADR ingest produced no locked decisions; fall back to discuss-phase for this phase.` and exit with `/bm:discuss-phase {N}` guidance.
...
6. Commit with `bm-sdk query commit "docs(${padded_phase}): generate context from ADR ingest" --files "${phase_dir}/${padded_phase}-CONTEXT.md"` and set `context_content`; continue to step 5.
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /bm:plan-phase {X} ${GSD_WS}:

  /bm:discuss-phase {X} ${GSD_WS}
  ```
...
AI_PHASE_CFG=$(bm-sdk query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
◆ Note: This phase appears to involve AI system development.
  Consider running /bm:ai-integration-phase {N} before planning to:
  - Select the right framework for your use case
  - Research its docs and best practices
  - Design an evaluation strategy

  Continue planning without AI-SPEC? (non-blocking — /bm:ai-integration-phase can be run after)
```

Use AskUserQuestion with options:
- "Continue — plan without AI-SPEC"
- "Stop — I'll run /bm:ai-integration-phase {N} first"

If "Stop": Exit with `/bm:ai-integration-phase {N}` reminder.
...
<files_to_read>
- {context_path} (USER DECISIONS from /bm:discuss-phase)
...
**If `RESEARCH_ONLY=true`:** the user invoked `/bm:plan-phase --research-phase <N>` for research-only mode. Do **not** continue to Section 5.5+ (validation strategy, planner, plan-checker, verification, gaps, bounce, post-planning-gaps). Print the research-complete summary and exit cleanly:
...
Re-run /bm:plan-phase ${PHASE} to plan the phase using this research,
or /bm:plan-phase ${PHASE} --research to refresh research and plan.
```
...
SECURITY_ASVS=$(bm-sdk query config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(bm-sdk query config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$(bm-sdk query config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(bm-sdk query config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(bm-sdk query roadmap.get-phase "${PHASE}" 2>/dev/null)
...
```bash
AUTO_ACTIVE=$(bm-sdk query check auto-mode --pick active 2>/dev/null || echo "true")
```
...
```
Continue to step 6. (For an interactive design session instead, run `/bm:ui-phase {N}`
...
`/bm:ui-phase {N} ${GSD_WS}` — generate UI design contract before planning
───────────────────────────────────────────────
Also available:
- `/bm:plan-phase {N} --skip-ui ${GSD_WS}` — plan without UI-SPEC (not recommended for frontend phases)
```
...
```bash
PHASE_SECTION=$(bm-sdk query roadmap.get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
1. Re-run: `/bm:plan-phase {PHASE} --research ${GSD_WS}`
2. Disable Nyquist with the exact command:
   `bm-sdk query config-set workflow.nyquist_validation false`
...
Check config:
```bash
PATTERN_MAPPER_CFG=$(bm-sdk query config-get workflow.pattern_mapper 2>/dev/null || echo "true")
```
...
**MVP_MODE:** ${MVP_MODE} (when true, Read `${CLAUDE_PLUGIN_ROOT}/references/planner-mvp-mode.md` for vertical-slice rules — lazy, only on MVP runs; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Read `${CLAUDE_PLUGIN_ROOT}/references/planner-mvp-mode.md` and follow its vertical-slice planning rules. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>

<downstream_consumer>
Output consumed by /bm:execute-phase. Plans need:
...
`/bm:plan-phase {N} --chunked` resumes from the last successfully committed plan.

**Intended for new or in-progress chunked runs.** To recover plans already written by a prior
*non-chunked* run, use step 6's "Add more plans" or proceed directly to `/bm:execute-phase`
...
5. **Commit per-plan:**
   ```bash
   bm-sdk query commit "docs(${PADDED_PHASE}): plan ${plan_id} (chunked)" --files "${PHASE_DIR}/${plan_id}-PLAN.md"
   ```
...
3. **Stop** — exit; user can re-run `/bm:plan-phase {N}` to resume
...
**If "Split":** Use `/bm:phase --insert` to create the sub-phases, then replan each.
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/bm:debug` to investigate root causes."
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(bm-sdk query config-get workflow.plan_bounce_passes 2>/dev/null || echo "2")
BOUNCE_SCRIPT=$(bm-sdk query config-get workflow.plan_bounce_script 2>/dev/null | jq -r '.' 2>/dev/null || true)
```
...
```bash
bm-sdk query commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
GATE_CFG=$(bm-sdk query config-get workflow.context_coverage_gate 2>/dev/null || echo "true")
if [ "$GATE_CFG" != "false" ]; then
  GATE_RESULT=$(bm-sdk query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}")
...
2. **Re-run the gate:** `bm-sdk query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}"`.
...
```bash
bm-sdk query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
bm-sdk query roadmap.annotate-dependencies "${PHASE_NUMBER}"
```
...
```bash
bm-sdk query commit "docs(${PADDED_PHASE}): create phase plan" --files "${PHASE_DIR}"/*-PLAN.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
POST_PLANNING_GAPS=$(bm-sdk query config-get workflow.post_planning_gaps --default true 2>/dev/null || echo true)
...
re-run `/bm:plan-phase --gaps` to add plans, or proceed to execute-phase as-is.
...
3. **Sync chain flag with intent** — if user invoked manually (no `--auto` and no `--chain`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]] && [[ ! "$ARGUMENTS" =~ --chain ]]; then
     bm-sdk query config-set workflow._auto_chain_active false || true
   fi
   ```
...
if ([[ "$ARGUMENTS" =~ --auto ]] || [[ "$ARGUMENTS" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  bm-sdk query config-set workflow._auto_chain_active true
fi
```
...
- **If `PLAN_COUNT` <= 2 (cheap phase):** silent auto-advance.
  ...
  **Handle execute-phase return:**
  - **PHASE COMPLETE** → Display final summary:
    ...
    Next: /bm:discuss-phase ${NEXT_PHASE} --auto ${GSD_WS}
    ```
...
/bm:execute-phase {X} ${GSD_WS}

───────────────────────────────────────────────────────────────

**Also available:**
- cat .planning/phases/{phase-dir}/*-PLAN.md — review plans
- /bm:plan-phase {X} --research — re-research first
- /bm:review --phase {X} --all — peer review plans with external AIs
- /bm:plan-phase {X} --reviews — replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/bm:plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/bm:plan-phase N --skip-research
```
</windows_troubleshooting>
