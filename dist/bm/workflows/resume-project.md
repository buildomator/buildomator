...
**If `planning_exists` is false:** This is a new project - route to /bm:new-project
</step>
...
- This is the primary resumption source — structured data from `/bm:pause-work`
...
📋 [N] pending todos — /bm:capture --list to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/bm:execute-phase {phase} ${GSD_WS})
   OR
1. Discuss Phase 3 context (/bm:discuss-phase 3 ${GSD_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/bm:plan-phase 3 ${GSD_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show direct next command:
  ...
  **Also available:**
  - `/bm:discuss-phase [N] ${GSD_WS}` — gather context first
  - `/bm:plan-phase --research-phase [N] ${GSD_WS}` — investigate unknowns
