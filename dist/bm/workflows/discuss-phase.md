...
@${CLAUDE_PLUGIN_ROOT}/references/universal-anti-patterns.md
</required_reading>
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/bm:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
Use /bm:progress ${GSD_WS} to see available phases.
```
Exit workflow.
...
if [ -f "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/plugins/cache/gsd-plugin/current}/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Check if a SPEC.md (from `/bm:spec-phase`) exists for this phase. SPEC.md locks requirements before implementation decisions.
...
**Note:** SPEC.md files named `AI-SPEC.md` (from `/bm:ai-integration-phase`) are excluded — different purpose.
</step>
...
If raw spikes/sketches exist but no findings skill, note: `⚠ Unpackaged spikes/sketches detected — run /bm:spike --wrap-up or /bm:sketch --wrap-up to make findings available.`
...
```bash
TODO_MATCHES=$(bm-sdk query todo.match-phase "${PHASE_NUMBER}")
```
...
Read `@${CLAUDE_PLUGIN_ROOT}/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
**Also available:** `--chain` for auto plan+execute after; `/bm:plan-phase ${PHASE} --skip-research ${GSD_WS}` to plan without research; `/bm:ui-phase ${PHASE} ${GSD_WS}` for UI design contracts; review/edit CONTEXT.md before continuing.
```
</step>
...
```bash
bm-sdk query commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
bm-sdk query commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
