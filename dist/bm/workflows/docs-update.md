...
- bm:gsd-doc-writer — Writes and updates project documentation files
- bm:gsd-doc-verifier — Verifies factual claims in docs against the live codebase
</available_agent_types>
...
```
Agent(
  subagent_type="bm:gsd-doc-writer",
...
Display note:
```
To fix failures automatically: /bm:docs-update (runs generation + fix loop)
To regenerate all docs from scratch: /bm:docs-update --force
```
...
3. Re-run /bm:docs-update to regenerate clean docs
```
...
```bash
bm-sdk query commit "docs: generate project documentation" \
...
```
Run `/bm:docs-update --verify-only` to fact-check generated docs against the codebase.
```
