...
```
Usage: /bm:undo --last N | --phase NN | --plan NN-MM
...
```
Working tree has uncommitted changes. Commit or stash them before running /bm:undo.
```
...
**Also available:**
- `/bm:execute-phase ${PHASE}` — re-execute if needed
- `/bm:undo --last 1` — undo the revert itself if something went wrong
