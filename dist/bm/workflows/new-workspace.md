...
```bash
INIT=$(bm-sdk query init.new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /bm:workspace --new --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /bm:workspace --new --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd "$TARGET_PATH"
  /bm:new-project    # Initialize GSD in the workspace
```
...
- options:
  - "Yes — run /bm:new-project" → tell user to `cd "$TARGET_PATH"` first, then run `/bm:new-project`
