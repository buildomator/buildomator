...
Check config:
```bash
AI_PHASE_ENABLED=$(bm-sdk query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```

**If `AI_PHASE_ENABLED` is `false`:**
```
AI phase is disabled in config. Enable via /bm:settings.
```
Exit workflow.
...
Recommended: run /bm:discuss-phase {N} first to capture framework preferences.
...
**Parse flags from `$ARGUMENTS`:** `--refresh` (force-regenerate, re-run framework-selector + downstream pipeline), `--view` (print existing to stdout and exit).

**If exists AND no `--refresh` AND no `--view`:** auto-use existing. Emit a one-line notice and exit cleanly:

```
AI-SPEC.md already exists for Phase {N}, using it.
...
**If exists AND `--refresh`:** continue to step 5 (re-run with existing as baseline).
...
**If selector fails or returns empty:** Exit with error — "Framework selection failed. Re-run /bm:ai-integration-phase {N} or answer the framework question in /bm:discuss-phase {N} first."
...
Next step:
  /bm:plan-phase {N}   — planner will consume AI-SPEC.md
```
