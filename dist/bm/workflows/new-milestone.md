...
- Tell the user to complete/archive the previous milestone first, then rerun `/bm:new-milestone --reset-phase-numbers ${GSD_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/bm:plan-phase` behavior. To change the default, use `/bm:settings`.
...
", subagent_type="bm:gsd-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
`/bm:discuss-phase [N] ${GSD_WS}`, gather context and clarify approach

Also: `/bm:plan-phase [N] ${GSD_WS}`, skip discussion, plan directly
```
...
- [ ] User knows next step: `/bm:discuss-phase [N] ${GSD_WS}`
