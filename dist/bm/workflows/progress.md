...
Run /bm:new-project to start a new project.
```

Exit.

If missing STATE.md: suggest `/bm:new-project`.

**If ROADMAP.md missing but PROJECT.md exists:**

This means a milestone was completed and archived. Go to **Route F** (between milestones).

If missing both ROADMAP.md and PROJECT.md: suggest `/bm:new-project`.
</step>
...
## Pending Todos
- [count] pending — /bm:capture --list to review

## Active Debug Sessions
- [count] active — /bm:debug to continue
...
**If `INCOMPLETE_PHASE` is non-empty:** emit a one-line resume notice in the routing output and route to `/bm:execute-phase ${INCOMPLETE_PHASE}` instead of running Step 1's current-phase routing.
...
`/bm:execute-phase ${INCOMPLETE_PHASE} ${GSD_WS}`
...
Review: `/bm:audit-uat ${GSD_WS}` — full cross-phase audit
Resume testing: `/bm:verify-work {phase} ${GSD_WS}` — retest specific phase
```
...
`/bm:plan-phase {phase-number} ${GSD_WS}`
...
`/bm:discuss-phase {phase}` — gather context and clarify approach

---

**Also available:**
- `/bm:ui-phase {phase}` — generate UI design contract (recommended for frontend phases)
- `/bm:plan-phase {phase}` — skip discussion, plan directly
- `/bm:discuss-phase {phase}` — include assumptions check before planning
...
`/bm:discuss-phase {phase} ${GSD_WS}` — gather context and clarify approach

---

**Also available:**
- `/bm:plan-phase {phase} ${GSD_WS}` — skip discussion, plan directly
- `/bm:discuss-phase {phase} ${GSD_WS}` — include assumptions check before planning
...
`/bm:plan-phase {phase} --gaps ${GSD_WS}`

---

**Also available:**
- `/bm:execute-phase {phase} ${GSD_WS}` — execute phase plans
- `/bm:verify-work {phase} ${GSD_WS}` — run more UAT testing
...
`/bm:verify-work {phase} ${GSD_WS}` — resume testing from where you left off

---

**Also available:**
- `/bm:audit-uat ${GSD_WS}` — full cross-phase UAT audit
- `/bm:execute-phase {phase} ${GSD_WS}` — execute phase plans
...
`/bm:discuss-phase {Z+1}` — gather context and clarify approach

---

**Also available:**
- `/bm:ui-phase {Z+1}` — generate UI design contract (recommended for frontend phases)
- `/bm:plan-phase {Z+1}` — skip discussion, plan directly
- `/bm:verify-work {Z}` — user acceptance test before continuing
...
`/bm:discuss-phase {Z+1} ${GSD_WS}` — gather context and clarify approach

---

**Also available:**
- `/bm:plan-phase {Z+1} ${GSD_WS}` — skip discussion, plan directly
- `/bm:verify-work {Z} ${GSD_WS}` — user acceptance test before continuing
...
`/bm:complete-milestone ${GSD_WS}`

---

**Also available:**
- `/bm:verify-work ${GSD_WS}` — user acceptance test before completing milestone
...
- Phase complete but next phase not planned → offer `/bm:plan-phase [next] ${GSD_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/bm:resume-work ${GSD_WS}`
</step>
...
- Check 2 (orphaned handoff): `Read the handoff file(s) and resume from where work was paused: /bm:resume-work ${GSD_WS}`
...
- Check 1 (STATE inconsistency): `Run /bm:verify-work ${PHASE} ${GSD_WS} to reconcile state`
</step>
...
- [ ] Smart routing: /bm:execute-phase if plans exist, /bm:plan-phase if not
