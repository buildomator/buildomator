...
- `/bm:add-todo Add auth token refresh` → title = "Add auth token refresh"
...
```bash
slug=$(bm-sdk query generate-slug "$title" --raw)
```
...
```bash
bm-sdk query commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
...
2. Add another todo
3. View all todos (/bm:capture --list)
```
</step>
