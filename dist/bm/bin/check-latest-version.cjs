...
/**
 * Deterministic latest-version check for /bm:update (#2992).
 *
 * The /bm:update workflow's check_latest_version step was previously
...
function checkLatestVersion(opts = {}) {
  const defaultSpawn = () => cp.spawnSync('npm', ['view', PACKAGE_NAME, 'version'], {
    ...
    // entire /bm:update workflow indefinitely (#2993 CR). 15s is generous
