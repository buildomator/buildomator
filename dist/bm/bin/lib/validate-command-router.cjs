...
function routeValidateCommand({ verify, args, cwd, raw, parseNamedArgs, output, error }) {
  ...
  if (subcommand === 'consistency') {
    ...
    const RECOMMENDATIONS = {
      [STATES.HEALTHY]: null,
      [STATES.WARNING]: 'Context is approaching the fracture zone — consider /bm:thread to continue in a fresh window.',
      [STATES.CRITICAL]: 'Reasoning quality may degrade past 70% utilization (fracture point). Run /bm:thread now to preserve output quality.',
    };
    let classified;
    try {
