...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /bm:new-project to set up.',
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/bm:profile-user --refresh` with session data to populate.';
  ...
  // location so /bm:profile-user --refresh stops re-creating the legacy
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by GSD from ${dataSource}. Run \`/bm:profile-user --refresh\` to update.`,
    '',
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /bm:profile-user to unlock Developer Profile.';
