...
// Sentinel phases (0 drafts, 999 backlog) are never-on-roadmap by convention,
// so both "missing dir" and "missing roadmap entry" warnings are wrong for
// them, and they are never part of sequential numbering. Matches the parsed
// leading integer: 0 or 999. Letter-led ids are never sentinels.
...
function cmdValidateHealth(cwd, options, raw) {
  ...
  if (!fs.existsSync(planBase)) {
    addIssue('error', 'E001', '.planning/ directory not found', 'Run /bm:new-project to initialize');
    output({
      status: 'broken',
      errors,
      warnings,
  ...
  if (!fs.existsSync(projectPath)) {
    addIssue('error', 'E002', 'PROJECT.md not found', 'Run /bm:new-project to create');
  } else {
  ...
  if (!fs.existsSync(roadmapPath)) {
    addIssue('error', 'E003', 'ROADMAP.md not found', 'Run /bm:new-milestone to create roadmap');
  }

  // ─── Check 4: STATE.md exists and references valid phases ─────────────────
  if (!fs.existsSync(statePath)) {
    addIssue('error', 'E004', 'STATE.md not found', 'Run /bm:health --repair to regenerate', true);
      ...
      if (!normalizedValid.has(ref) && !normalizedValid.has(padded)) {
        ...
        if (normalizedValid.size > 0) {
          addIssue(
            ...
            'Review STATE.md manually before changing it; /bm:health --repair will not overwrite an existing STATE.md for phase mismatches'
          );
        }
      }
    }
  ...
  if (!fs.existsSync(configPath)) {
    addIssue('warning', 'W003', 'config.json not found', 'Run /bm:health --repair to create with defaults', true);
    ...
    try {
      ...
      addIssue('error', 'E005', `config.json: JSON parse error - ${err.message}`, 'Run /bm:health --repair to reset to defaults', true);
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (configParsed.workflow && configParsed.workflow.nyquist_validation === undefined) {
        addIssue('warning', 'W008', 'config.json: workflow.nyquist_validation absent (defaults to enabled but agents may skip)', 'Run /bm:health --repair to add key', true);
        if (!repairs.includes('addNyquistKey')) repairs.push('addNyquistKey');
      }
      if (configParsed.workflow && configParsed.workflow.ai_integration_phase === undefined) {
        addIssue('warning', 'W016', 'config.json: workflow.ai_integration_phase absent (defaults to enabled — run /bm:ai-integration-phase before planning AI system phases)', 'Run /bm:health --repair to add key', true);
    ...
    if (hasResearch && !hasValidation) {
      ...
      try {
        ...
        if (researchContent.includes('## Validation Architecture')) {
          addIssue('warning', 'W009', `Phase ${e.name}: has Validation Architecture in RESEARCH.md but no VALIDATION.md`, 'Re-run /bm:plan-phase with --research to regenerate');
        }
      } catch { /* intentionally empty */ }
    }
  }
        ...
        if (phaseCheckboxRe.test(roadmapContentFull)) {
          ...
          if (statusVal !== 'complete' && statusVal !== 'done') {
            addIssue('warning', 'W011',
              ...
              'Run /bm:progress to re-derive current position, or manually update STATE.md');
          }
        }
      }
      ...
      if (archivedVersions.length > 0) {
        ...
        if (missingFromRegistry.length > 0) {
          addIssue('warning', 'W018',
            ...
            'Run /bm:health --backfill to synthesize missing entries from archive snapshots',
            true);
      ...
      try {
        switch (repair) {
          ...
          case 'regenerateState': {
            ...
            stateContent += `- ${new Date().toISOString().split('T')[0]}: STATE.md regenerated by /bm:health --repair\n`;
          ...
          case 'backfillMilestones': {
            ...
            for (const ver of missingFromRegistry) {
              try {
                ...
                const entry = `## ${ver}${milestoneName && milestoneName !== ver ? ` ${milestoneName}` : ''} (Backfilled: ${today})\n\n**Note:** Synthesized from archive snapshot by \`/bm:health --backfill\`. Original completion date unknown.\n\n---\n\n`;
