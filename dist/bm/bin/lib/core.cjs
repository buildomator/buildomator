...
// ─── [PLUGIN PATCH] Plugin-root resolution helpers ─────────────────────────
// These are plugin-only additions that let bin/ code locate the plugin root
// whether it's installed as a Claude Code plugin (CLAUDE_PLUGIN_ROOT env var)
// or run from the legacy ~/.claude/get-shit-done layout.
...
const CONFIG_DEFAULTS = {
  ...
  security_enforcement: true, // workflow.security_enforcement — threat-model-anchored security verification via /bm:secure-phase
...
function loadConfig(cwd, options = {}) {
  ...
  try {
    ...
    return {
      ...
      auto_advance: get('auto_advance', { section: 'workflow', field: 'auto_advance' }) ?? true,
